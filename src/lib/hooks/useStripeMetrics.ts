  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        // Clear cache and recheck when page becomes visible
        if (user?.email) {
          stripeKeyCache.delete(user.email);
        }
        checkStripeKey();
      }
    };

    // Listen for visibility changes (page refresh, tab focus)
    document.addEventListener('visibilitychange', handleVisibilityChange);

    // Also handle actual page refresh
